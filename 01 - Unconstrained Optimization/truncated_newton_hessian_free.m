function [xk , fk , gradfk_norm , k, xseq , btseq , rate_convergence ] = truncated_newton_hessian_free(x0, f, gradf, kmax, tolgrad, c1, rho, btmax)
% TRUNCATED_NEWTON_HESSIAN_FREE Hessian - Free Truncated Newton method with backtracking (Armijo ) and no preconditioning
%
% [xk , fk , gradfk_norm , k, xseq , btseq , convergence_rate ] = truncated_newton_hessian_free (x0 , f, gradf , ...
% kmax ,tolgrad , c1 , rho , btmax )
%
% This function implements the Hessian - Free Truncated Newton method for unconstrained optimization
% of a function f: R^n -> R. It approximates the Hessian - vector product using finite differences ,
% making it suitable for problems where the Hessian is expensive or impossible to compute directly .
% It uses a backtracking line search based on the Armijo condition to ensure sufficient
% decrease in the function value at each iteration . Preconditioning is NOT used in this version .
%
% INPUTS :
...
% rate_convergence - Estimate of the rate of convergence ( average of estimated orders in the last iterations )
...
%% Hessian - Free Approximation Parameter
sigma = sqrt(eps); % Square root of machine precision , used in finite difference
approximation
...
while (k < kmax ) && ( gradfk_norm >= tolgrad )
    ...
    % Define the Hessian - vector product operator approximated using finite differences
    % Hv(p) approximates Hessian (f(xk)) * p
    Hv = @(p) (gradf(xk + sigma * p) - gradf(xk)) / sigma ;

    % Compute the search direction pk by approximately solving Hv(pk) = -gradf (xk)
    % using Conjugate Gradient method ( cg_curvtrun_hessian_free function , defined below )
    pk = cg_curvtrun_hessian_free(Hv, -gradfk, zeros(n ,1), etak);

    % Descent direction check : ensure the computed direction is a descent direction
    % If not , fall back to steepest descent direction (- gradf (xk))
    if gradfk' * pk >= 0
